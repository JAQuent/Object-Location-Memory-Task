---
title: 'Trial Generation for fMRI version of OLM task: Code & reasoning'
...
We get the angle $\theta$ simply by using the inverse of $tan()$, which is $atan()$, which can be done like this: 
...
### Experimental Design
1 run of encoding task (6 objects x 2 trial ~ 3 min), 1 run of retrieval task with 1 task block (6 objects x 2 trials ~ 5 min) = 10 minutes in total. The participant starts from one of the other object locations during this part of the experiment. In addition, there will also be control trials. Each object serves as a starting location twice in total. 
...
The experiment will start with 30 encoding trials, followed by the 15 retrieval + 14 control trials, an additional 30 'encoding' trials as well as another run of retrieval & control trials so that we can examine whether the memory task is crucial for the grid-signal to arise but at the same time we didn't want to overtrain the subjects. 
...
# Save 
mainTask_locations_offset <- mainTask_locations
...
```{r mainTask_timings}
cuePeriod    <- 2 # seconds
delayPeriods <- seq(from = 1, to = 2, length.out = 6) 
ITIPeriods   <- seq(from = 1, to = 2, length.out = 6) 
...
# Save in controlTrials1
controlTrials1 <- controlTrials
```

And now I repeat this with the $15^\circ$ offset locations.
...
# Get the object location from run 1 
objectLocations <- ddply(encodingPhase_run1, c('targets'), summarise, x = x[1], z = z[1])
...
for(i in indexObjects){
  otherObjects <- indexObjects[!(indexObjects %in% i)] # Get all objects that are not the target
  controlTrials[controlTrials$targets == i, 'start_x'] <- mainTask_locations_offset[mainTask_locations_offset$targets %in% otherObjects, 'x']
  controlTrials[controlTrials$targets == i, 'start_z'] <- mainTask_locations_offset[mainTask_locations_offset$targets %in% otherObjects, 'z']
}
...
# Remove unnecessary
controlTrials$anglesDegrees <- NULL
controlTrials$anglesRadians <- NULL

# Save in controlTrials2 & bind together
controlTrials2 <- controlTrials
controlTrials  <- rbind(controlTrials1, controlTrials2)
```
...
encoding1 <- data.frame(block_num = c(rep(1, nrow(tempVar))),
                       run = c(tempVar$run),
...
encoding2 <- data.frame(block_num = c(rep(3, nrow(tempVar))),
                  run = c(tempVar$run),
                  targets = c(tempVar$targets),
                  start_x = c(tempVar$start_x),
                  start_z = c(tempVar$start_z),
...
# Retrieval 1
tempVar <- retrievalPhase[retrievalPhase$run == 1, ]
retrieval1 <- data.frame(block_num = c(rep(1, nrow(tempVar))),
                       run = c(tempVar$run),
                       targets = c(tempVar$targets),
                       start_x = c(tempVar$start_x),
                       start_z = c(tempVar$start_z),
                       ...
                       delay = c(tempVar$delay),
                       ITI = c(tempVar$ITI),
                       trialType = c(rep('retrieval', nrow(tempVar))),
                       speedForward = speedForward,
                       rotationSpeed = rotationSpeed,
                       messageToDisplay = -1)

# Retrieval 2
tempVar <- retrievalPhase[retrievalPhase$run == 2, ]
retrieval2 <- data.frame(block_num = c(rep(1, nrow(tempVar))),
                       run = c(tempVar$run),
                       targets = c(tempVar$targets),
                       start_x = c(tempVar$start_x),
                       start_z = c(tempVar$start_z),
...
# Control 1
tempVar <- controlTrials[controlTrials$run == 1, ]
control1 <- data.frame(block_num = c(rep(1, nrow(tempVar))),
                       run = c(tempVar$run),
                       targets = c(tempVar$targets),
                       start_x = c(tempVar$start_x),
                       start_z = c(tempVar$start_z),
                       ...
                       delay = c(tempVar$delay),
                       ITI = c(tempVar$ITI),
                       trialType = c(rep('control', nrow(tempVar))),
                       speedForward = speedForward,
                       rotationSpeed = rotationSpeed,
                       messageToDisplay = -1)
# Control 2
tempVar <- controlTrials[controlTrials$run == 2, ]
control2 <- data.frame(block_num = c(rep(1, nrow(tempVar))),
                       run = c(tempVar$run),
                       targets = c(tempVar$targets),
                       start_x = c(tempVar$start_x),
                       start_z = c(tempVar$start_z),
...
mainTask <- rbind(encoding1, # Encoding 1
                  retrieval1[1:15, ], # retrieval run 1.1
                  control1[1:15, ], # control half 1.1
                  retrieval1[16:30, ], # retrieval run 1.2
                  control1[16:30, ], # control half 1.2
                  encoding2, # Encoding 2
                  retrieval2[1:15, ], # retrieval run 2.1
                  control2[1:15,], # control half 2.1
                  retrieval2[16:30, ], # retrieval run 2.2
                  control2[16:30,]) # control half 2.2
...
mainTask[30, 'messageToDisplay']  <- 0 # Before retrieval/control 1
mainTask[90, 'messageToDisplay']  <- 1 # Before encoding 2
mainTask[120, 'messageToDisplay'] <- 0 # Before retrieval/control 2
...
### Checks
```{r check_plot1, fig.height = 6, fig.width = 7, echo = FALSE}
# Convert to factor
...
As second test, I check whether all paths are sampled for all three types:

```{r check_plot2, fig.height = 4, fig.width = 12, echo = FALSE}
# Plot
ggplot(mainTask, aes(x = start_x, y = start_z, xend = object_x, yend = object_z, colour = trialType, shape = run)) + 
  facet_wrap(.~trialType) +
  geom_segment() +
  labs(title = "Were the correct path used?", x = '', y = '')
```

That is case as the patterns created by the paths are the same for all three trial types. 